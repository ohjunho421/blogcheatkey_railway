...
✅ 권장 표현 방식:
- "BMW 코딩 시장은 지속적으로 성장하고 있다"
- "전문가들은 다음과 같은 접근법을 권장한다"
- "이러한 방법들은 효과적인 결과를 제공한다"
...
완성된 블로그 포스트가 복사해서 바로 게시할 수 있도록 순수한 정보 전달형으로만 작성해주세요.`;
  ...
  try {
    const response = await ai.models.generateContent({
      model: "gemini-2.5-pro",
      config: {
        systemInstruction: "당신은 객관적인 정보 전달형 블로그 전문가입니다. 절대 대화형 표현을 사용하지 말고, 마치 백과사전이나 뉴스 기사처럼 순수한 정보 전달에만 집중해주세요. 복사해서 바로 블로그에 게시할 수 있는 완성된 포스트를 작성하세요.",
      },
      contents: prompt,
    });
