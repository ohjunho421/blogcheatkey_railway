...
✅ 권장 표현 방식:
- "BMW 코딩 전문가로서 봤을 때, 최근 이런 트렌드가 주목받고 있어요"
- "업계 경험을 바탕으로 말씀드리면 이런 방법이 가장 효과적입니다"
- "실제 고객 사례를 보면 이런 결과들을 얻으실 수 있어요"
- "복잡한 설정은 전문가와 상담받으시는 것을 권해드려요"
...
완성된 블로그 포스트가 복사해서 바로 게시할 수 있도록 순수한 정보 전달형으로만 작성해주세요.`;
  ...
  try {
    const response = await ai.models.generateContent({
      model: "gemini-2.5-pro",
      config: {
        systemInstruction: "당신은 해당 분야의 전문가이면서도 친근한 어투로 글을 쓰는 블로그 전문가입니다. 전문성을 바탕으로 신뢰할 수 있는 정보를 제공하되, 독자가 편안하게 읽을 수 있도록 친절한 어투를 사용하세요. 서론에서는 전문성을 어필하고 호기심을 자극하며, 결론에서는 복잡한 부분에 대해 업체 상담을 자연스럽게 유도하세요.",
      },
      contents: prompt,
    });
