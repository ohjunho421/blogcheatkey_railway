  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const baseInstructions = [
        ...
        `   → 작성 중 실시간으로 글자수를 세면서 작성하세요. 1800-1900자를 목표로!`,
        ``,
        `🎯 키워드 "${keyword}"의 완전한 형태를 정확히 5-7회 사용해주세요. (4회 이하나 8회 이상 절대 금지)`,
        `   → 작성하면서 키워드를 몇 번 사용했는지 세면서 작성하세요. 6회를 목표로!`,
        ``,
        `🎯🎯 매우 중요: 키워드를 의미 단위로 분해한 각 단어를 15-18회씩 사용 (16회 목표)`,
        `   → 예시:`,
        `     • "벤츠엔진경고등" = "벤츠" 16회 + "엔진" 16회 + "경고등" 16회`,
        `     • "냉각수부동액" = "냉각수" 16회 + "부동액" 16회  👈 각각 별도 카운트!`,
        `     • "타이어교체비용" = "타이어" 16회 + "교체" 16회 + "비용" 16회`,
        `   → 작성 중 각 단어를 쓸 때마다 "냉각수: 14회, 부동액: 13회..." 이렇게 세세요!`,
        `   → 키워드 구성 단어가 11회 이하면 너무 적음, 19회 이상이면 과다입니다.`,
        ``,
        `🚨 중요: 키워드가 아닌 다른 모든 단어는 반드시 14회 이하로 제한해주세요!`,
        `   → 같은 단어를 15회 이상 쓰지 마세요. 10-12회 정도가 적당합니다.`,
      ...
      if (previousAnalysis && attempt > 1) {
        ...
        if (attempt === 2) {
          ...
          seoSuggestions.push(`   • 글자수: 1800-1900자 목표 (1700-2000자 범위)`);
          seoSuggestions.push(`   • 완전 키워드 "${keyword}": 6회 (5-7회 범위)`);
          seoSuggestions.push(`   • 키워드 구성 단어: 각 16회 (15-18회 범위)`);
          seoSuggestions.push(`   • 일반 단어: 14회 이하 (10-12회 권장)`);
          seoSuggestions.push(`⚠️ 이번이 마지막 시도입니다. 각 단어를 쓸 때마다 횟수를 세면서 작성하세요!`);
        }
      }
