  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      const seoSuggestions = attempts > 1 ? [
        ...
        `키워드 "${keyword}"의 각 구성 요소를 15-17회씩 정확히 사용하세요`,
        `예: "벤츠엔진경고등" → "벤츠" 15-17회, "엔진" 15-17회, "경고등" 15-17회`,
        `키워드가 글에서 가장 많이 출현하는 단어가 되어야 함`,
        `공백 제외 1700-1800자 엄수`
      ] : [
        `키워드 "${keyword}"의 각 구성 요소를 15-17회씩 정확히 포함하세요`,
        `개별 구성 요소들을 각각 15-17회 사용해야 합니다`,
      ...
      if (customMorphemesArray.length > 0) {
        ...
        seoSuggestions.push(
          `추가 형태소를 포함하면서도 키워드 "${keyword}"의 각 구성 요소가 15-17회씩 출현하는 조건을 반드시 맞춰주세요`
        );
        if (attempts > 1) {
          seoSuggestions.push(`이전 시도에서 누락된 추가 형태소가 있었거나 키워드 "${keyword}"의 구성 요소들 출현 횟수가 부족했습니다. 두 조건을 모두 만족하세요.`);
        }
      }
