...
export async function generateStrictMorphemeContent(
  ...
  try {
    ...
    // 최대 강화된 형태소 제한 SEO 지침
    const seoSuggestions = [
      `공백 제외 정확히 1700-2000자 범위 안에서 글을 작성해주세요.`,
      `키워드 "${keyword}"의 완전한 형태를 정확히 5회만 사용해주세요. (5회 초과 금지)`,
      `키워드를 구성하는 각 형태소를 15-19회 사용해주세요. (절대 20회 이상 금지)`,
      `키워드 형태소라도 20회 이상 사용하면 검색엔진 스팸으로 판정됩니다.`,
      `다른 모든 단어는 14회 이하로 엄격히 제한해주세요.`,
      `🚨 절대 금지: 키워드 전체 5회 초과, 키워드 형태소 20회 이상, 일반 단어 14회 이상`,
      `동의어와 유사어를 적극 활용하여 단어 반복을 최소화해주세요.`
    ];
