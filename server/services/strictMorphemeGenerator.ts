...
export async function generateStrictMorphemeContent(
  ...
  try {
    ...
    // 자연스러운 SEO 지침 생성 (형태소 빈도 조정)
    const seoSuggestions = [
      `공백 제외 정확히 1700-2000자 범위 안에서 글을 작성해주세요. (1700자 미만이나 2000자 초과 절대 금지)`,
      `키워드 "${keyword}"의 완전한 형태를 정확히 5-7회 자연스럽게 사용해주세요.`,
      `키워드를 구성하는 각 형태소를 정확히 8-12회씩 자연스럽게 사용해주세요. (과도한 반복 금지)`,
      `다른 모든 단어는 10회 미만으로 제한해주세요. (자연스러운 빈도 유지)`,
      `서론 500-600자, 본론 800-900자, 결론 200-300자로 분량을 정확히 배치해주세요.`,
      `단어 반복이 부자연스럽지 않도록 동의어와 유사어를 적절히 활용해주세요.`
    ];
