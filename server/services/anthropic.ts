...
【충격적 사실 제시형】 구체적 예시:
- "${keyword} 관련해서 놀라운 통계가 나왔는데요. 10명 중 8명이 잘못된 방법을 사용하고 있다고 하더라고요"
- "최근 발표된 자료를 보니 ${keyword} 때문에 발생하는 손실이 연간 수십억 원에 달한다고 해요"
- "업계 전문가들이 ${keyword}에 대해 완전히 잘못 알려진 정보 때문에 골머리를 앓고 있다고 하네요"
...
- 연구 자료에 명시된 기관이나 출처만 정확히 인용
- 연구 자료에 없는 통계나 수치는 절대 사용하지 마세요 
- 연구 자료에 없는 기관명(예: 소비자 연구원)은 언급하지 마세요
- "최근 연구에 따르면", "업계 보고서에서는", "전문가들에 따르면" 같은 일반적 표현만 사용
- 연구 자료에 실제로 언급된 출처만 정확히 인용
- 가짜 통계나 수치는 절대 사용하지 마세요
...
🎯 서론 작성 필수 지침:
위의 7가지 서론 패턴 중 반드시 하나를 선택하여 서론을 작성하세요.
매번 다른 패턴을 사용하여 독자에게 새로운 느낌을 주세요.
서론 분량은 전체 글의 25-30%로 충분히 매력적으로 작성하세요.
