...
🚨 중요: 키워드 사용 방식 🚨:
- "${keyword}" 키워드는 반드시 완전한 형태로 15-17회 사용
- 개별 구성 요소(예: "벤츠", "엔진", "경고등")를 각각 카운트하지 않음
- 전체 키워드 "${keyword}"가 하나의 단위로 15-17회 나타나야 함
- 이 키워드가 글에서 가장 빈번하게 사용되는 단어여야 함
...
- 키워드 "${keyword}" 완전한 형태로 15-17회 정확히 포함 (개별 구성요소 아님)
- 키워드가 글에서 가장 많이 출현하는 단어가 되어야 함 (다른 어떤 단어보다 많아야 함)
