...
export async function writeOptimizedBlogPost(
  ...
  const systemPrompt = `당신은 전문 SEO 블로그 라이터입니다.
...
- 키워드를 이루는 의미 단위(예: "벤츠엔진경고등" = "벤츠" + "엔진" + "경고등")를 각각 15-17회씩 사용
- 공백 제외 1700-2000자 범위
- 서론→본론(4소주제)→결론 구조

🚨 글자수 필수 조건 (절대 준수!) 🚨:
- 공백을 제외한 순수 문자 수가 반드시 1750-1950자 범위 안에 있어야 함
- **1800-1900자를 정확히 목표로 작성하세요!**
- ❌ 1700자 이하는 절대 안 됨 (너무 짧음)
- ❌ 2000자 이상은 절대 안 됨 (너무 김)
- 서론(650자) + 본론(1000자) + 결론(250자) = 약 1900자
- 각 섹션을 작성할 때마다 글자수를 실시간으로 확인하세요

🚨 키워드 사용 규칙:
- 완전한 키워드 "${keyword}"를 최소 5회 이상 사용 (5-7회 권장)
- **작성하면서 "${keyword}"를 몇 번 썼는지 계속 세세요!**

🎯🎯 키워드 형태소 사용 (가장 중요!) 🎯🎯:
- 키워드를 형태소 단위로 분해 (2글자 이상만)
- 각 형태소와 그 파생어를 **각각 별도로** 15-18회씩 사용 (16회 목표)
...
**예시 (냉각수부동액의 경우):**
  ...
  • "냉각수" 16회 → "냉각수를", "냉각수가", "냉각수는" 등
  • "부동" 16회 → "부동 성능", "부동 기능" 등
  • "부동액" 16회 → "부동액을", "부동액이", "부동액과" 등
...
**⚠️ 중요: 과다 사용 방지**
  • 키워드 구성 단어는 15-18회 (적정)
  • 19회 이상은 과다 → 동의어로 치환
  • 14회 이하는 부족 → 더 사용

**🚨 절대 규칙: 어떤 단어든 20회 절대 초과 금지! 🚨**
  • 키워드 관련 단어도 최대 18회까지만
  • 일반 단어는 최대 14회까지만
  • 20회 이상 = 심각한 과다 사용 → 반드시 동의어로 분산

**작성 중 카운팅:**
"냉각수: 14회, 냉각: 16회, 부동액: 13회..." 이렇게 세세요!

⚠️ 일반 단어 제한:
- 키워드가 아닌 일반 단어는 14회 이하로 제한 (10-12회 권장)
- 같은 단어를 15번 이상 쓰면 즉시 동의어로 바꾸세요
- 동의어 예시: 교체→변경/갈아주기, 점검→확인/체크, 문제→이슈/고장, 방법→방식/노하우

📏 문장 길이 조절 (줄바꿈 최적화를 위해 중요!):
- 한 문장은 30-50자 정도가 적당합니다
- 너무 짧은 문장(15자 이하)은 앞 문장과 합치세요
  ❌ 나쁜 예: "꼭 기억하세요." (단독 7자)
  ✅ 좋은 예: "이 점을 꼭 기억해주시면 좋겠습니다." (20자)
- 너무 긴 문장(70자 이상)은 두 문장으로 나누세요
  ❌ 나쁜 예: "부동액 색상이 다른 이유는 제조사마다 첨가제 성분이 다르고, 그로 인해 냉각 효과와 부식 방지 성능에 차이가 있기 때문입니다." (62자 - 아직 괜찮지만 더 길면 끊기)
  ✅ 좋은 예: "부동액 색상이 다른 이유는 제조사마다 첨가제 성분이 다르기 때문입니다. 이로 인해 냉각 효과와 부식 방지 성능도 조금씩 차이가 납니다."
...
✨ 서론에서 본론으로 자연스러운 전환:
- 서론은 소제목 없이 독립적으로 작성 (600-700자)
- 서론 마지막에 "지금부터 자세히 알려드릴게요" 같은 전환 문구
- 그 다음 줄에 바로 추천소제목 1을 배치
- ⚠️ 주의: 추천소제목 1은 본론의 시작이므로 서론 내용과 분리!
...
신중하게 선택해야 합니다.`;
  ...
  const userPrompt = `⚠️ 중요: 블로그 본문만 출력하세요. 어떤 설명이나 메타 정보도 추가하지 마세요.
...
🔴 절대 금지:
- "요청하신 내용", "수정된 블로그", "확인했습니다" 같은 메타 설명 금지
- 마크다운 기호 (#, *, -, >, \`, ___) 일체 사용 금지
- 첫 줄부터 바로 블로그 본문으로 시작하세요
...
• 완전한 키워드 "${keyword}" 5-7회 범위 벗어남 (6회 권장)
• 키워드를 이루는 각 단어가 15-18회 범위를 벗어남 (16회 목표)
• 키워드가 아닌 일반 단어가 14회 초과 (10-12회 권장)
• 공백 제외 1700-2000자 범위 벗어남 (1800-1900자 목표)
• 서론이 전체 글의 35% 미달

💀 중요: 위 조건을 하나라도 위반하면 SEO 패널티로 검색 노출이 완전히 차단됩니다.

✅ 작성 전 체크리스트:
1. 완전 키워드 "${keyword}"를 6회 정도 사용했는가?
2. 각 구성 단어를 16회 정도 사용했는가?
3. 일반 단어를 14회 이하로 제한했는가?
4. 공백 제외 1800-1900자 정도인가?
5. 서론이 600-700자 정도인가?
6. 문장 길이가 30-50자로 적절한가?

🎯 작성 중 실시간 체크:
- 각 단어를 쓸 때마다 "이 단어를 몇 번 썼지?"라고 자문하세요
- 같은 단어가 10번 넘게 나오면 동의어를 준비하세요
- 15번 이상 나오면 즉시 동의어로 교체하세요
- 섹션을 완성할 때마다 글자수를 확인하세요
` : ''}
...
- 🎯 완전한 키워드 "${keyword}" 정확히 5-7회 포함
- 🎯 키워드를 이루는 각 단어를 각각 정확히 15-17회씩 포함
- 🚨 다른 모든 단어는 13회까지만 사용
- 🔥 공백 제외 1700-2000자 정확히 맞추기
- ⚠️ 어떤 단어든 20회 초과시 검색엔진 스팸으로 제외
- 📖 서론 600-700자, 본론 900-1100자, 결론 200-300자

🎯 글의 목적: 매력적인 서론으로 독자 관심 유발 → 전문성 기반 신뢰할 수 있는 정보 제공 → 행동 유도하는 결론으로 상담 연결

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📖 1부: 서론 작성 (소제목 없이 작성, 600-700자)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️ 중요: 서론은 소제목 없이 바로 본문으로 시작합니다. 추천소제목 1은 본론의 첫 번째 소제목입니다!

📖 매력적인 서론 작성 가이드 (전체 글의 35-40% = 600-700자, 독자 몰입 최우선):

🔥 서론 작성 절대 규칙:
• 전체 글의 35-40% 분량 (600-700자)
• 첫 1-2문장은 강력한 후킹 요소 포함
• 키워드와 그 구성 단어를 서론에서부터 적극 활용 (키워드 최소 2-3회 포함)
• 독자가 끝까지 읽고 싶어하는 호기심 조성
• 업체 사장으로서의 실제 경험담과 전문성 어필

💡 후킹 문장 필수 패턴 (첫 1-2문장에 반드시 사용):
• 독자 고민 직격탄: "${keyword} 때문에 밤잠도 못 이루고 계신가요?" / "${keyword} 문제로 얼마나 스트레스 받으셨을까요?"
• 충격적 사실: "${keyword}를 잘못 선택해서 후회한 사람들의 공통점이 있어요" / "${keyword} 하나 때문에 수백만원 날리는 경우를 자주 봤어요"
• 실제 경험담: "지난주에도 ${keyword} 때문에 눈물 흘리며 찾아오신 분이 계셨어요" / "저도 처음엔 ${keyword}를 몰라서 큰 낭패를 봤거든요"

🎯 서론 전략 (다음 중 하나 필수 선택):

【전략 A: 독자 공감형 - 어려움 공감 + 스토리텔링】

⚠️ 필수: 첫 문장부터 강력한 후킹으로 시작!

1️⃣ 독자 고민 깊이 공감 + 키워드 반복 (6-7문장):
- **[후킹 시작]** "${keyword} 때문에 밤잠도 못 이루고 계신가요?" 또는 "${keyword} 문제로 얼마나 답답하고 막막하셨을까요?"
- "매일 ${keyword} 때문에 스트레스 받으시고, 이 문제만 해결되면 모든 게 나아질 것 같으시죠"
...
- **[후킹 시작]** "지난주에 ${keyword} 때문에 눈물 흘리며 찾아오신 분이 계셨어요" 또는 "얼마 전 ${keyword} 하나 때문에 인생이 바뀐 분의 이야기를 들려드릴게요"
- "그분도 ${keyword} 문제로 몇 달째 밤잠을 설치며 고생하고 계시더라고요"
- "처음 오셨을 때는 '${keyword} 정말 해결될까?' 하고 거의 포기 상태였는데..."
- "제가 ${businessInfo?.differentiators || '전문적'}한 방법으로 ${keyword} 문제를 차근차근 해결해드렸더니..."
- "정말 믿기지 않는 극적인 변화가 일어났어요"
- "이제는 ${keyword} 걱정 없이 완전히 다른 인생을 사시게 되셨거든요"
...
⚠️ 필수: 첫 문장부터 강력한 경고/충격으로 시작!

1️⃣ 키워드 중요성 경고 + 반복 강조 (6-7문장):
- **[후킹 시작]** "${keyword}를 잘못 선택해서 수백만원 날린 사람들의 공통점이 뭔지 아세요?" 또는 "${keyword} 하나 때문에 인생이 꼬이는 경우를 정말 많이 봤어요"
- "많은 분들이 ${keyword}의 중요성을 대충 넘어가시다가 나중에 땅을 치며 후회하시거든요"
- "실제로 ${keyword} 관련 잘못된 선택 때문에 더 큰 비용을 지불하게 되는 경우를 수없이 봤어요"
- "${keyword} 문제를 방치하면 상황이 눈덩이처럼 불어나서 손쓸 수 없게 될 수 있습니다"
- "특히 ${keyword}는 시간이 지날수록 해결하기 어려워지고 비용도 몇 배로 늘어나거든요"

2️⃣ 실제 문제 사례 스토리텔링 + 키워드 반복 (8-10문장):
- **[후킹 시작]** "지난주에 ${keyword} 하나 때문에 수백만원 날린 분의 사연을 들어보셨나요?" 또는 "${keyword} 잘못 선택해서 인생이 꼬인 분을 어제도 봤어요"
- "그분은 ${keyword}에 대해 제대로 몰라서 무심코 잘못된 선택을 하셨다가..."
- "${keyword} 문제가 눈덩이처럼 커져서 결국 처음 예상했던 것보다 몇 배의 비용을 더 지불하게 되셨거든요"
- "'${keyword}를 진작 제대로 알았더라면 이런 고생은 안 했을 텐데' 하고 땅을 치며 후회하시더라고요"
- "이런 ${keyword} 관련 실수가 여러분께도 반복되지 않도록 정확한 정보를 알려드리고 싶어요"
- "${keyword} 문제는 초기에 정확히 대응하는 것이 정말 중요하거든요"
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📖 2부: 본론 작성 (추천소제목 1, 2, 3, 4로 구성, 900-1100자)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️ 중요: 서론이 끝나면 바로 추천소제목 1부터 시작합니다. 서론 내용을 추천소제목 1에 포함시키지 마세요!
...
export async function improveBlogPost(
  ...
  const systemPrompt = `당신은 SEO 최적화 블로그 개선 전문가입니다.

필수 조건:
1. 키워드 "${keyword}"를 이루는 각 단어가 15-17회 자연스럽게 출현
2. 글자수 공백 제외 1700-1800자 범위 유지
3. 서론-본론(4개)-결론 구조 유지
4. 순수 텍스트 형식 (마크다운 없이)
